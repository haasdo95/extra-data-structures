...
TEST(hq_test, test_cons) {
    ...
    ASSERT_DEATH(HashQueue hWrong{wrong}, "");
...
TEST(hq_test, test_obj_simple) {
    HashQueue<A, std::string> q{[](const A& a) { return a.id(); }};
    q.push(1.0, 0, 0);
    q.push(2.0, 0, 1);
    q.push(3.0, 1, 0);
    q.push(4.0, 1, 1);
